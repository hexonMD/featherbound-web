...
import { printMasterUrl } from "@/lib/upscale";
...
export async function POST(req: NextRequest) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    // Collection collage: the asset was composed + uploaded at checkout time, so metadata
    // already carries the print URL + SKU. Just place the Prodigi order.
    if (md.kind === "collage" && md.assetUrl && a.line1 && a.country) {
      const reference = `${md.title || "Field Guide"} — Field Guide (${md.sizeLabel || md.sizeId})`;
      try {
        await createProdigiOrder(
          { prodigiSku: md.prodigiSku, image: md.assetUrl, assetUrl: md.assetUrl },
          recipient,
          reference,
          { paperType: "EMA", substrateWeight: "200gsm" }
        );
      } catch (err) {
        console.error("[prodigi] collage fulfilment failed", err);
      }
    ...
    if (art && a.line1 && a.country) {
      ...
      // Build the print asset: upscale + compose onto a field-guide plate at the ordered size.
      // Falls back to the on-site plate if the pipeline isn't configured.
      const assetUrl = await printMasterUrl(
        art.image,
        art.slug,
        art.speciesCommon,
        art.speciesSci,
